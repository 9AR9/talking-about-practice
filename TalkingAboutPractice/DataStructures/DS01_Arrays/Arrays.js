...
describe("Arrays", function () {
    ...
    it("should demonstrate confusing issue with new Array()", function () {
        ...
        var moreThanOneIntegerParameterNowUsesNumbersAsValuesInsteadOfLength = new Array(40, 99);

        expect(moreThanOneIntegerParameterNowUsesNumbersAsValuesInsteadOfLength.length).toEqual(2);
    });
    ...
    it("should sort array of numbers", function() {
        ...
        integers.sort(function (a, b) { return a - b }); // A negative result means the first value is lower than the second for sort; results in numeric order

        expect(integers).toEqual([7, 8, 9, 100, 200]);

        integers.sort(function (a, b) { return b - a }); // The opposite comparison gives us reverse numeric order
    ...
    it("should sort array of objects", function () {
        ...
        cars.sort(function (a, b) { return a.year - b.year }); // We can sort based on a specific object property with same numeric approach

        expect(cars).toEqual([{ type: "Saab", year: 2001 }, { type: "BMW", year: 2010 }, { type: "Volvo", year: 2016 }]);

        cars.sort(function(a, b) { // Textual sort on an object property requires explicit same-cased comparison
            var x = a.type.toLowerCase();
            var y = b.type.toLowerCase();
            if (x < y) { return -1 }; // Negative return value means a is lower than b in alphabetical order
            if (x > y) { return 1 }; // Postitive return value means b is lower than a in alphabetical order
            return 0; // Zero return value means a and b are the same value
        });

        expect(cars).toEqual([{ type: "BMW", year: 2010 }, { type: "Saab", year: 2001 }, { type: "Volvo", year: 2016 }]);
    });
